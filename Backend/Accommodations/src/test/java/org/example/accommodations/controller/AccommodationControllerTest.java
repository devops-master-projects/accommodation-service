...
class AccommodationControllerTest {
    ...
    void create_okWithHostRole() throws Exception {
        ...
        mockMvc.perform(post("/api/accommodations")
                ...
                .andExpect(jsonPath("$.location.city").value("Novi Sad"))
                .andExpect(jsonPath("$.urlPhotos", hasSize(2)))
                .andExpect(jsonPath("$.amenities[*].name", containsInAnyOrder("WiFi", "Parking")));

        ArgumentCaptor<String> valueCap = ArgumentCaptor.forClass(String.class);
        verify(kafkaTemplate, atLeastOnce()).send(org.mockito.ArgumentMatchers.eq("accommodation-events"),
                org.mockito.ArgumentMatchers.anyString(), valueCap.capture());
        assertThat(valueCap.getAllValues().get(0)).contains("AccommodationCreated");
    }
    ...
    void get_autoConfirm_guestAllowed() throws Exception {
        ...
        var createRes = mockMvc.perform(post("/api/accommodations")
                        .with(user("host-user").roles("host"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(createBody))
                .andExpect(status().isOk())
                .andReturn()
        ...
        mockMvc.perform(get("/api/accommodations/{id}/auto-confirm", id)
                        .with(user("guest-user").roles("guest")))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("Security: public GET /api/accommodations requires no authentication")
    void getAll_publicNoAuth() throws Exception {
        mockMvc.perform(get("/api/accommodations"))
                .andExpect(status().isOk());
    }
}
